...
final class SpellbookManager {
    ...
    func castSpell(_ spell: Spell, for user: User) {
        ...
        switch spell.effect {
        case .doubleXP: user.isDoubleXpNextTask = true
        case .doubleGold: user.activeBuffs[spell.effect] = Date().addingTimeInterval(600)
        }
    }
...
final class GameLogicManager {
    ...
    private func checkForLevelUp(user: User) -> Bool {
        ...
        if user.totalXP >= xpForNextLevel {
            user.level += 1; user.currency += 100; user.runes += 1
